...
export default class SmilesDrawer {
     ...
     * @param {?CallableFunction} errorCallback The function called on error.
     */
    drawFromLayout(layoutJson, target, theme = 'light', successCallback = null, errorCallback = null) {
        try {
            let parseTree = Parser.parse(layoutJson.smiles);
            this.drawParseTree(parseTree, target, theme, null, successCallback, layoutJson);
        }
        catch (err) {
            if (errorCallback) {
                errorCallback(err);
     ...
     */
    drawParseTree(parseTree, target, theme, weights, callback, presetLayout = null) {
        if (target === null || target === 'svg') {
            let svg = this.drawer.draw(parseTree, null, theme, weights, false, [], false, presetLayout);
            ...
            let canvas = this.svgToCanvas(this.drawer.draw(parseTree, null, theme, weights, false, [], false, presetLayout));
            if (callback) {
                callback(canvas);
            }
        }
        else if (target === 'img') {
            let img = this.svgToImg(this.drawer.draw(parseTree, null, theme, weights, false, [], false, presetLayout));
            if (callback) {
                callback(img);
            }
        }
        else if (target instanceof HTMLImageElement) {
            this.svgToImg(this.drawer.draw(parseTree, null, theme, weights, false, [], false, presetLayout), target);
            if (callback) {
                callback(target);
            }
        }
        else if (target instanceof SVGElement) {
            this.drawer.draw(parseTree, target, theme, weights, false, [], false, presetLayout);
            if (callback) {
                callback(target);
            }
        }
            ...
            elements.forEach((element) => {
                ...
                if (tag === 'svg') {
                    this.drawer.draw(parseTree, element, theme, weights, false, [], false, presetLayout);
                    ...
                    this.svgToCanvas(this.drawer.draw(parseTree, null, theme, weights, false, [], false, presetLayout), element);
                    if (callback) {
                        callback(element);
                    }
                }
                else if (tag === 'img') {
                    this.svgToImg(this.drawer.draw(parseTree, null, theme, weights, false, [], false, presetLayout), element);
                    if (callback) {
                        callback(element);
                    }
                }
