...
export default class DrawerBase {
    ...
    initDraw(data, themeName, infoOnly, highlight_atoms, presetLayout = null) {
        this.data = data;
    ...
    /**
     * Runs the overlap-resolution passes that clean up the automatic layout
     * (rotating subtrees/rings and relaxing coiled chains). Not used when
     * drawing from a preset layout, since that layout is already overlap-free
     * and should be left as-is to match its source depiction.
     */
    resolveOverlaps() {
    ...
    getBridgedRingRings(ringId) {
        ...
        let recurse = (r) => {
            ...
            for (let i = 0; i < ring.neighbours.length; i++) {
                ...
                if (involvedRings.indexOf(n) === -1 && n !== r && RingConnection.isBridge(this.ringConnections, this.graph.vertices, r, n, id => this.getRing(id))) {
                    recurse(n);
                }
            }
        };
    ...
    isPartOfBridgedRing(ringId) {
        for (let i = 0; i < this.ringConnections.length; i++) {
            if (this.ringConnections[i].containsRing(ringId) && this.ringConnections[i].isBridge(this.graph.vertices, id => this.getRing(id), this.ringConnections)) {
                return true;
            }
        }
     ...
     */
    position() {
        if (this.presetLayout && this.applyPresetLayout()) {
            return;
        }
    ...
    /**
     * Applies externally-supplied atom coordinates (this.presetLayout) instead
     * of running the automatic layout algorithm. Used to redraw a molecule from
     * a pre-computed 2D depiction (e.g. one generated by RDKit) while still
     * reusing this drawer's own ring/stereo/rendering logic.
     *
     * @returns {Boolean} Whether the preset layout was applied. False means the
     *                    layout didn't match the parsed molecule and the caller
     *                    should fall back to the automatic layout.
     */
    applyPresetLayout() {
        const layout = this.presetLayout;
        const atomIdxToVertexId = this.graph.atomIdxToVertexId;

        if (!Array.isArray(layout.atoms) || layout.atoms.length !== atomIdxToVertexId.length) {
            console.warn('SmilesDrawer: preset layout atom count does not match the parsed SMILES; falling back to automatic layout.');
            return false;
        }

        for (let i = 0; i < layout.atoms.length; i++) {
            const vertex = this.graph.vertices[atomIdxToVertexId[i]];

            if (vertex.value.element.toUpperCase() !== layout.atoms[i].element.toUpperCase()) {
                console.warn('SmilesDrawer: preset layout element mismatch at atom index ' + i + '; falling back to automatic layout.');
                return false;
            }
        }

        // Scale the layout's pixel coordinates so bond lengths match this drawer's opts.bondLength.
        let bondLengthSum = 0;
        let bondCount = 0;
        for (let i = 0; i < layout.bonds.length; i++) {
            const bond = layout.bonds[i];
            const a = layout.atoms[bond.begin_atom_index];
            const b = layout.atoms[bond.end_atom_index];
            bondLengthSum += Math.hypot(a.x - b.x, a.y - b.y);
            bondCount++;
        }
        const scale = bondCount > 0 ? this.opts.bondLength / (bondLengthSum / bondCount) : 1;

        const vertexIdToAtomIndex = new Map();
        for (let i = 0; i < layout.atoms.length; i++) {
            const vertex = this.graph.vertices[atomIdxToVertexId[i]];
            vertex.setPosition(layout.atoms[i].x * scale, layout.atoms[i].y * scale);
            vertex.positioned = true;
            vertexIdToAtomIndex.set(vertex.id, i);
        }

        // Authoritative positions for implicit hydrogens the layout's source
        // (e.g. RDKit) computed but couldn't list as a regular atom, keyed by
        // their heavy-atom parent's atom_index. Preferred over the heuristic
        // below whenever available, since it reflects the actual source
        // depiction rather than a guess.
        const implicitHByParentAtomIndex = new Map(
            (Array.isArray(layout.implicit_hydrogens) ? layout.implicit_hydrogens : [])
                .map(h => [h.parent_atom_index, h])
        );
        ...
        for (let i = 0; i < this.graph.vertices.length; i++) {
            ...
            const parent = this.graph.vertices[vertex.parentVertexId];
            ...
            const parentAtomIndex = vertexIdToAtomIndex.get(parent.id);
            const implicitH = parentAtomIndex === undefined ? undefined : implicitHByParentAtomIndex.get(parentAtomIndex);
            if (implicitH) {
                vertex.setPosition(implicitH.x * scale, implicitH.y * scale);
                vertex.positioned = true;
                continue;
            }

            const siblingPositions = parent.neighbours
                .filter(id => id !== vertex.id && this.graph.vertices[id].positioned)
                .map(id => this.graph.vertices[id].position);
            ...
            vertex.positioned = true;
            this.resolvePresetOverlap(vertex, parent);
        }

        // ring.center is normally set while createRing() walks the spanning
        // tree; with a preset layout we compute it directly as a centroid.
        for (let i = 0; i < this.rings.length; i++) {
            const ring = this.rings[i];
            const centroid = ring.members
                .reduce((sum, id) => sum.add(this.graph.vertices[id].position.clone()), new Vector2(0, 0))
                .divide(ring.members.length);

            ring.center = centroid;
    ...
    /**
     * Rotates a heuristically-placed vertex (one the preset layout didn't
     * cover, see applyPresetLayout()) around its parent until it clears
     * every already-positioned vertex. The rest of the preset layout is
     * fixed and must not move, so this only ever repositions the single
     * vertex passed in - never a whole subtree.
     *
     * Uses fontSizeLarge (an atom label's rough glyph size) rather than
     * resolveSecondaryOverlaps()'s 0.1*bondLength "exactly coincident"
     * threshold: that stricter bound still let the reported bug's H/N pair
     * (3.66px apart on a 30px bond) through as "not colliding", even though
     * two label glyphs that close visibly overlap.
     *
     * @param {Vertex} vertex The heuristically-placed vertex to check/move.
     * @param {Vertex} parent Its already-positioned parent, used as the pivot.
     */
    resolvePresetOverlap(vertex, parent) {
        ...
        const radius = vertex.position.distance(parent.position);
        const baseAngle = Math.atan2(vertex.position.y - parent.position.y, vertex.position.x - parent.position.x);

        // ponytail: fixed 15-degree steps around the parent, first angle
        // that clears wins; if none do (a very crowded local geometry),
        // keep the last tried position - no worse than the unchecked
        // heuristic this replaces. Revisit with a proper local search only
        // if that fallback is ever seen to look bad in practice.
        for (let step = 1; step <= 23; step++) {
            const angle = baseAngle + step * (Math.PI / 12);
            vertex.setPosition(
                parent.position.x + radius * Math.cos(angle),
                parent.position.y + radius * Math.sin(angle)
            );
    ...
    createRing(ring, center = null, startVertex = null, previousVertex = null) {
        ...
        if (ring.isBridged) {
            ...
            let orderedMembers = [];
            ring.eachMember(this.graph.vertices, v => orderedMembers.push(v), startVertexId, (previousVertex) ? previousVertex.id : null);

            let n = orderedMembers.length;
            let preFixed = orderedMembers.map(id => this.graph.vertices[id].positioned);
            let fixedCount = preFixed.reduce((acc, fixed) => acc + (fixed ? 1 : 0), 0);

            // Extended (3+ atom) arc fusions can leave two or more members of
            // this ring already positioned by a smaller, already-drawn ring
            // before we get here - and that smaller ring's real angular
            // spacing rarely matches this ring's uniform central angle. Blindly
            // continuing a uniform step from a single start vertex would
            // silently create a kink (wrong bond length/angle) right where the
            // walk re-meets those other fixed atoms. When two or more
            // genuinely fixed anchors exist, recompute the angular step per
            // gap of unfixed members so it lands exactly on the next
            // already-fixed atom's real angle. A lone fixed vertex (the usual
            // case, including the start vertex of a plain 2-atom fusion being
            // temporarily unfixed by the caller) has nothing to reconcile
            // against, so it keeps the original uniform-step behaviour.
            if (fixedCount >= 2) {
                let angleOf = id => Vector2.subtract(this.graph.vertices[id].position, center).angle();

                // Rotate the start so the walk begins at a genuinely fixed
                // member; every wrap-around gap then has a real anchor on
                // both ends.
                let firstFixed = preFixed.indexOf(true);
                let rotated = orderedMembers.slice(firstFixed).concat(orderedMembers.slice(0, firstFixed));
                let rotatedFixed = preFixed.slice(firstFixed).concat(preFixed.slice(0, firstFixed));
                let baseAngle = angleOf(rotated[0]);
                ...
                while (i < n) {
                    ...
                    let j = i;
                    while (j < n && !rotatedFixed[j]) {
                        j++;
                    }

                    let prevAngle = (i === 0) ? baseAngle : angleOf(rotated[i - 1]);
                    let nextAngle = (j === n) ? baseAngle + MathHelper.twoPI : angleOf(rotated[j]);

                    while (nextAngle <= prevAngle) {
                        nextAngle += MathHelper.twoPI;
                    }

                    let hops = j - i + 1;
                    let step = (nextAngle - prevAngle) / hops;

                    for (let k = i; k < j; k++) {
                        let a2 = prevAngle + step * (k - i + 1);
                        let vertex = this.graph.vertices[rotated[k]];
                        vertex.setPosition(center.x + Math.cos(a2) * radius, center.y + Math.sin(a2) * radius);
                        vertex.angle = a2;
                        vertex.positioned = true;
                    }
        ...
        for (let i = 0; i < orderedNeighbours.length; i++) {
            ...
            if (vertices.length === 2) {
                ...
                // Extended fusion: a short bond-path shared with a much larger ring.
                // Whichever of the two rings is smaller keeps its shape for the
                // shared arc; the larger ring's polygon walk must skip those
                // already-fixed atoms rather than recompute them. Depending on
                // which ring happened to be positioned first, that smaller ring
                // may be `ring` (already positioned) or `neighbour` (about to be
                // positioned), so only unfix the arc when neighbour is the one
                // that should be correcting it.
                ring.isFused = true;
                neighbour.isFused = true;

                let arc = RingConnection.getOrderedVertices(this.ringConnections, this.graph.vertices, ring.id, neighbour.id);
                let vertexA = this.graph.vertices[arc[0]];
                let vertexB = this.graph.vertices[arc[1]];
                ...
                if (neighbour.getSize() <= ring.getSize()) {
                    for (let j = 1; j < arc.length; j++) {
                        this.graph.vertices[arc[j]].positioned = false;
                    }
                }
    ...
    resolveSecondaryOverlaps(scores) {
        // Two atoms closer than this are drawn on top of each other.
        let minSeparationSq = (0.1 * this.opts.bondLength) ** 2;
            ...
            if (scores[i].score > this.opts.overlapSensitivity) {
                ...
                if (vertex.isTerminal()) {
                    ...
                    if (closest) {
                        ...
                        // Two terminal substituents on the same atom each pick the
                        // other as `closest` and each rotate away using the other's
                        // pre-move position, so they can swap sides and land on the
                        // exact same point - a worse overlap than the one being
                        // resolved. Undo a rotation that ends on top of the atom it
                        // was fleeing; every other rotation is left alone.
                        let saved = vertex.position.clone();

                        vertex.position.rotateAwayFrom(closestPosition, vertexPreviousPosition, MathHelper.toRad(20));

                        if (vertex.position.distanceSq(closest.position) < minSeparationSq) {
                            vertex.position.x = saved.x;
                            vertex.position.y = saved.y;
                        }
                    }
                }
            }
